...
find_new_edges_tergm <- function(net_pred, net_list) {
  if (length(net_list) < 2) stop("❌ net_list에는 최소 2개의 네트워크가 필요합니다.")
  
  # --- 기준 노드셋 추출 ---
  base_nodes <- network.vertex.names(net_list[[1]])
  
  # --- 엣지리스트를 node name 기준으로 변환하는 함수 ---
  ...
  cat("✅ 새로 등장한 엣지 수:", nrow(edges_new), "\n")
  return(edges_new)
}
...
plot.network(
  simp1,
  label = label,
  vertex.cex = sqrt(deg + 1) * 0.8,   # indegree에 따른 노드 크기
  displaylabels = TRUE,
  vertex.col = "skyblue",
  vertex.border = 0.3,
  edge.col = ifelse(
    ...
    "red", "grey60"                   # 🔴 새 엣지는 빨간색, 나머지는 회색
  ),
  label.cex = 0.6,
  edge.lwd = 0.3,
  main = "TPU Network : 2030 ~ (New Edges Highlighted)",
  displayisolates = FALSE,            # 고립 노드 제거
  mode = "fruchtermanreingold"        # 레이아웃
)
